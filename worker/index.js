  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === '/resolve_channel' && request.method === 'POST') {
        ...
        if (!tgData.ok) return jsonResponse({ error: 'تأكد أن البوت مشرف في القناة' }, 400);
        const chat = tgData.result;
        return jsonResponse({ chat_id: chat.id, title: chat.title || username, username: username });
      }
      if (path === '/publish' && request.method === 'POST') {
        const body = await request.json();
        const { chat_id, text, btn_name, btn_url, btn_icon } = body;
        if (!chat_id || !text || !btn_name || !btn_url) return jsonResponse({ error: 'جميع الحقول مطلوبة' }, 400);
        const buttonText = `${btn_icon || ''} ${btn_name}`.trim();
        const payload = { chat_id, text, reply_markup: { inline_keyboard: [[{ text: buttonText, url: btn_url }]] }, parse_mode: 'HTML' };
        const tgResp = await fetch(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        const tgData = await tgResp.json();
